...
def send_email_list(gateway_name, connection_name_email, approved_list_mismatch, ra_cidrs_type):
  ...
  # Email body
  if ra_cidrs_type == "approved_golden":
  ...
  elif ra_cidrs_type == "approved_list":
    message['Subject'] = "Aviatrix - Route Approval - Approved CIDRs Mismatch detected"
    body1 = "Total number of approved CIDRs changed for gateway: " + gateway_name + " and connection: " + connection_name_email + ".\n\n"
    body2 = "\n\nThe following CIDRs are present in Golden List but not Approved by Route Approval feature:\n" + approved_list_mismatch
  elif ra_cidrs_type == "gw_approved_golden":
    message['Subject'] = "Aviatrix - Route Approval - Approved CIDRs Mismatch detected"
    body1 = "Total number of approved CIDRs changed for gateway: " + gateway_name + " (Gateway Mode Route Approval).\n\n"
    body2 = "\n\nThe following CIDRs are approved by Route Approval feature but not present in Golden List:\n" + approved_list_mismatch
  elif ra_cidrs_type == "gw_approved_list":
    message['Subject'] = "Aviatrix - Route Approval - Approved CIDRs Mismatch detected"
    body1 = "Total number of approved CIDRs changed for gateway: " + gateway_name + " (Gateway Mode Route Approval).\n\n"
    body2 = "\n\nThe following CIDRs are present in Golden List but not Approved by Route Approval feature:\n" + approved_list_mismatch
...
def csv_file_creation(file_option, gateway_name, file_data, connection_name):
  ...
  if connection_name == "Gateway-Mode":
    #print("Gateway Mode file creation")
    if file_option == "approved_cidr_list":
      csv_filename = gateway_name+"_"+connection_name+"_approved_cidr_list_date_"+str_current_datetime+".csv"
      ...
      csv_filename = gateway_name+"_"+connection_name+"_total_approved_cidr_date_"+str_current_datetime+".csv"
      file_path = os.path.join(folder, csv_filename)
      ...
      csv_filename = gateway_name+"_"+connection_name+"_pending_cidr_list_date_"+str_current_datetime+".csv"
      file_path = os.path.join(folder, csv_filename)
      ...
      csv_filename = gateway_name+"_"+connection_name+"_total_pending_cidr_date_"+str_current_datetime+".csv"
      file_path = os.path.join(folder, csv_filename)
...
#--------------- Check Whether Total Number of Approved CIDRs changed - Connection Mode ---------------#
def check_total_approved(gateway_name, list_of_connections, count_number_of_connections):
  goldenfile_approved_number = 0
...
#--------------- Check Whether Total Number of Approved CIDRs changed - Connection Mode ---------------#
def check_total_approved_gw(gateway_name):
  #print("Gateway Mode function", gateway_name)
  goldenfile_approved_number = 0
  # Check the current number of Approved CIDRs
  # Define the file path
  file_path = "temp_files/" + gateway_name + "_Gateway-Mode_total_approved_cidr_date_" + formatted_current_date + ".csv"
  # Open the CSV file and read the number of total approved CIDRs
  # for Current Check
  with open(file_path, 'r') as csvfile:
    csvreader = csv.reader(csvfile)
  ...
  # for Golden List
  file_path = "golden_list/" + gateway_name + "_Gateway-Mode_approved_cidr_list_golden_list.csv"
  # Check whether the file for golden_list exists
  if os.path.exists(file_path):
    # Open the CSV file and read the number of total approved CIDRs
    with open(file_path, 'r') as csvfile:
  ...
  # Checking whether the total approved CIDR changed compared to golden file
  goldenfile_approved_number = int(goldenfile_approved_number) 
  currentcheck_approved_number = int(currentcheck_approved_number)
  if os.path.exists(file_path):
    if currentcheck_approved_number > goldenfile_approved_number:
      ...
      print("The number of approved CIDRs in the currrent-check of Route Approval feature:", currentcheck_approved_number)
      print("The number of approved CIDRs in the Golden List file:", goldenfile_approved_number)
      print("Total approved CIDR number is greater than Golden-List count for gateway: ", str(gateway_name))
      #send_email(gateway_name, list_of_connections[conn_name], goldenfile_approved_number, currentcheck_approved_number, "approved_ra")
    elif currentcheck_approved_number < goldenfile_approved_number:
      print("------------------------------------------------------------------------------------------------------------------------")
      print("The number of approved CIDRs in the currrent-check of Route Approval feature:", currentcheck_approved_number)
      print("The number of approved CIDRs in the Golden List file:", goldenfile_approved_number)
      print("Total approved CIDR number is lower than Golden-List count for gateway: ", str(gateway_name))
      #send_email(gateway_name, list_of_connections[conn_name], goldenfile_approved_number, currentcheck_approved_number, "approved_ra")
    else:
      #print("Total approved CIDR number has not changed for connection: ", list_of_connections[conn_name])
      pass
...
def check_missing_approved_cidrs_gw(gateway_name):
  file_path_golden_list = "golden_list/" + gateway_name + "_Gateway-Mode_approved_cidr_list_golden_list.csv"
  file_path_currentcheck_list = "temp_files/" + gateway_name + "_Gateway-Mode_approved_cidr_list_date_" + formatted_current_date + ".csv"
  try:
    end_row_goldenlist = count_rows_csv(file_path_golden_list)
    end_row_currentchecklist = count_rows_csv(file_path_currentcheck_list)
    end_row = max(end_row_goldenlist, end_row_currentchecklist)
    ...
    modified_golden_list_but_not_approved = sorted(only_in_goldenlist)
    modified_approved_but_not_golden__list = sorted(only_approved_but_not_in_goldenlist)
    #print(only_in_goldenlist)
    #print(only_approved_but_not_in_goldenlist)
    
    if len(only_in_goldenlist) != 0:
      print("------------------------------------------------------------------------------------------------------------------------")
      print("CIDR DETAILS - Gateway: {} in Gateway Mode.".format(gateway_name))
      print("The CIDRs present in Golden List {} file but not Approved by Route Approval feature:".format(file_path_golden_list))
      print(*modified_golden_list_but_not_approved, sep='\n')
      send_email_list(gateway_name, "Gateway-Mode", modified_golden_list_but_not_approved, "gw_approved_list")
      #print("Total number of approved CIDRs from Golden List file:",end_row_goldenlist-1)
    if len(only_approved_but_not_in_goldenlist) != 0:  
      print("------------------------------------------------------------------------------------------------------------------------")
      print("CIDR DETAILS - Gateway: {} in Gateway Mode.".format(gateway_name))
      print("The CIDRs approved but not present in Golden List file {}:".format(file_path_golden_list))
      print(*modified_approved_but_not_golden__list, sep='\n')
      send_email_list(gateway_name, "Gateway-Mode", modified_approved_but_not_golden__list, "gw_approved_golden")
      #print("Total number of approved CIDRs from Current Check file:",end_row_currentchecklist-1)
  except:
    print("Golden file {} does not exist".format(file_path_golden_list))
    pass
...
if trgw_list == True:
  ...
  list_of_connections = []
  if get_approval_info['results']['learned_cidrs_approval_mode'] == "gateway":
    pass
  else:
  ...
  if get_approval_info['results']['learned_cidrs_approval_mode'] == "gateway":
    approved = get_approval_info['results']['approved_learned_cidrs']
    pending = get_approval_info['results']['pending_learned_cidrs']
    ...
    csv_file_creation("approved_cidr_list", gateway_name, approved_mod, "Gateway-Mode")
    csv_file_creation("total_approved_cidr", gateway_name, str(approved_length), "Gateway-Mode")
    csv_file_creation("pending_cidr_list", gateway_name, pending_mod, "Gateway-Mode")
    csv_file_creation("total_pending_cidr", gateway_name, str(pending_length), "Gateway-Mode")
  else:
  ...
  if get_approval_info['results']['learned_cidrs_approval_mode'] == "gateway":
    check_total_approved_gw(gateway_name)
  else:
  ...
  if get_approval_info['results']['learned_cidrs_approval_mode'] == "gateway":
    check_missing_approved_cidrs_gw(gateway_name)
  else:
